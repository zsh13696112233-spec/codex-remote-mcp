...
```text
飞书群聊 ── HTTPS/WSS 443 长连接 ──▶ 角色任务配置中心（8091）
                                      │ 提交工作流
          ▼
...
配置中心可选接入一个飞书机器人。机器人使用官方 Java SDK 主动建立长连接，不要求为 `8080`、`8090` 或 `8091` 配置公网域名或入站端口。可在 8091 的“飞书机器人”页面保存应用凭据、选择固定任务并测试连接；群聊顶层 `@机器人 运行` 会启动该固定任务，任务进度、半自动流转按钮和任务助手完整回复会回到同一飞书话题。第一版全局最多运行一个机器人任务，终态话题仍可继续咨询或提出返工。
...
如需启用飞书机器人，请先按配置中心 README 创建并发布飞书应用、配置事件与权限，再在 8091 的“飞书机器人”页面测试并保存配置。`FEISHU_*` 环境变量只作为数据库尚无页面配置时的启动默认值。
...
- 飞书机器人只需要服务端主动访问飞书 HTTPS/WSS `443`；无需给本系统开放公网入站接口。
