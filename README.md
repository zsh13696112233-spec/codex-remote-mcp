...
FlowGram 支持“角色步骤 → IF 判断 → 下一角色步骤”的串行验收。判断继续使用前一步原会话，不通过最多自动修复配置次数，仍未通过则暂停并通知；用户可经任务助手确认追加一次修复。此能力不提供任意分支或跨步骤自动回退。升级须先更新 Python 中央及 Sidecar，再更新两个 Java 服务，完成后才启用带判断的新流程。配置与运行行为见各模块 README。
