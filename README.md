...
配置中心可选接入一个飞书机器人或钉钉机器人，两套适配均使用官方 Java SDK 主动建立长连接，不要求为 `8080`、`8090` 或 `8091` 配置公网域名或入站端口。同一部署只能启用一个平台，切换时必须先停用当前机器人。钉钉机器人固定一个任务定义，该任务的 SOP 只能选择一个已启用人员或群聊：群成员通过 @ 或回复控制，目标人员在单聊中直接控制；其他人员或群不能启动和控制。只有从钉钉发起的运行才回推钉钉。两套适配都支持固定任务启动、可靠进度补发和任务助手完整回复，并共享“同时最多一个机器人任务”的限制；钉钉个人模式使用文本或 Markdown，群模式可选互动卡片。
