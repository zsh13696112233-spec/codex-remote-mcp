...
如果中止、状态校验或事务执行失败，节点不会被部分重置，也不会扣减重跑额度。主界面只显示最新一次执行结果和图片，旧尝试保存在 SQLite 审计表和事件中。
...
本轮回归验证包括 Python 48 项测试、配置中心 4 项测试、监控中心 2 项测试，以及两个前端脚本的 JavaScript 语法检查。真实 MySQL 和远程 Codex App Server 的端到端验证仍需在对应运行环境中执行。
