...
```text
飞书群聊或钉钉指定人员/群聊 ── HTTPS/WSS 443 长连接 ──▶ 角色任务配置中心（8091）
                                                │ 提交工作流
          ▼
...
配置中心可选接入一个飞书机器人或钉钉机器人，两套适配均使用官方 Java SDK 主动建立长连接，不要求为 `8080`、`8090` 或 `8091` 配置公网域名或入站端口。同一部署只能启用一个平台，切换时必须先停用当前机器人。飞书仍固定一个任务定义；钉钉由多个任务定义分别绑定一个已启用人员或群聊，同一对象不能重复绑定。不同钉钉绑定可同时运行，群成员通过 @ 或回复控制本群任务，目标人员在单聊中控制本人的任务。任务定义可以配置每天固定北京时间或每隔 5–1440 分钟自动运行，并选择让网页或定时运行主动回推钉钉；钉钉个人模式使用文本或 Markdown，群模式可选互动卡片。
