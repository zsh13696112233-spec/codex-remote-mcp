...
- 配置中心 SOP 编辑区使用 `frontend/` 中的 React、TypeScript、Vite 与 FlowGram 自由布局编辑器；Maven 在构建阶段执行 `npm ci` 和前端构建，产物写入 `target/` 后打包。依赖版本及 npm 锁文件纳入版本控制，不提交依赖目录或生成资源。其他管理页面及监控中心仍使用原生 HTML、CSS、JavaScript，不为小改动引入框架。
- SOP 画布支持开始、角色步骤、角色步骤后的 IF 验收、结束组成的唯一串行链；前后端校验一致，服务端按连线排序。`nodeKey` 独立于步骤数据库 ID，`editorGraph` 仅用于配置编辑，不进入 Python 执行协议或回写历史运行快照。
...
- MCP 大文件传输的固定合并代码可在机器已登记 `allow_write` 和 `allow_full_access` 时请求完全访问；这是平台文件传输步骤，不执行安装包脚本或模型生成代码，不扩大安装助手权限。合并必须验证目录归属、路径边界、链接和完整哈希，拒绝覆盖内容不一致的已有文件。
- CLI + Skill 的平台验收可在同样的机器授权下，以完全访问执行经过路径校验的正式入口加固定 `--help`；入口仅为无额外参数的 EXE 或已有运行时加单个安装脚本路径，不执行登录或业务命令，不保存帮助输出，不扩大安装助手权限。帮助检查没有操作系统沙箱隔离，须使用受信安装包。
...
SOP 可在角色步骤之后配置 IF 验收关卡，仍为唯一串行主链，不是通用分支图。关卡在前一步原会话检查，未通过可按配置修复，超限或需人工决定时持久化暂停；不得以普通继续、跳过或倒计时绕过验收。自动修复次数与人工重跑额度分开，人工追加修复必须经过任务助手独立确认。验收配置进入运行快照；画布坐标不进入执行协议。
