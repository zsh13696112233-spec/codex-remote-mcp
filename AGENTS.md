...
- 需要终端调用的 CLI（含兼具 CLI 的 MCP）由平台固定代码写入执行账号的用户 PATH，要求已有写入及完全访问授权；校验目录归属、链接、命令冲突并去重，不修改机器级 PATH。纯 MCP 与 Skill 目录不加入 PATH，安装助手仍不得自行修改环境变量，MCP 注册及 Skill 示例保留绝对路径。
