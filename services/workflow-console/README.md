# 任务运行监控中心

端口 `8090` 的 Java Web 监控系统，用于展示一项 Codex 工作流的实时执行过程，并与该工作流的独立任务助手对话。浏览器访问 Java 服务，Java 服务代理端口 `8080` 的 Python 网关；本系统不连接配置中心的 MySQL 数据库。

## 技术栈

- Java 17
- Spring Boot 4.1
- Java `HttpClient`
- 原生 HTML、CSS 和 JavaScript

Java 根包为 `com.codexflow.console`，Web 控制器、异常映射和
工作流网关客户端分别维护。

## 页面能力

- 每个页面只查看 URL 指定的一项任务。
- 展示任务名称、整体状态、当前步骤和完成进度。
- 展示每一步的名称、角色、状态、起止时间和结果。
- 在对应步骤结果中内嵌预览图片；其他格式显示文件名、大小和下载入口。
- 将主监督的用户可读消息标记为“任务进度”，将咨询回复标记为“任务助手”。
- 在任务助手区域咨询最新进度，刷新页面后恢复完整对话。
- 助手可查询指定步骤的历史记录，并对已结束步骤发起独立只读核查；执行中的步骤只查已有记录。`chat.assistant.progress` 在原提问下显示正在查询/核查的步骤与执行版本，最终回答或失败后替换进度，刷新按事件恢复。咨询不自动返工或放行等待；不能读取原会话时回答会说明证据局限。
- 通过聊天提议停止任务、跳过步骤，或从指定步骤重跑到末尾；任务助手会展示 AI 总结的返工要求，必须另发“确认执行”才会生效。
- 半自动 SOP 在步骤成功后显示两分钟倒计时、“确认继续”和“保持等待”按钮；暂停后不再自动放行，并显示“确认继续”，刷新页面可从网关状态恢复等待或暂停。暂停不会触发返工，结果不符合要求时需在任务助手中说明修改点。
- 展示当前工作流的重跑上限、已用和剩余额度。
- 活动任务每两秒刷新一次；终态任务且没有待处理消息后每 15 秒轻量检查，发现重新运行后恢复正常刷新频率。
- 将内部状态转换为普通用户容易理解的中文。
- 不展示内部事件 JSON、执行机信息或会话编号。

本系统不提供任务编辑、提交、停止、尾部重跑或跳过的直接接口；这些控制仍只能通过任务助手二次确认。仅允许暂停或继续半自动 SOP 当前等待；添加、删除和修改步骤不受支持。

等待期间的任务消息会先保持等待，再交给助手；回复“确认继续”（兼容“继续”）或点击同名按钮继续。倒计时读取网关的实际截止时间，钉钉发送成功可能调整截止时间。所有模式业务步骤执行中均拒绝停止、跳过和返工，不预约操作；失败、完成后的原有返工保留。刷新恢复界面展示，不改变网关重启时的原有失败处理策略。

## 启动依赖

对话中显示从钉钉提交给 Codex 的输入图片，可点击查看原图。新增只读代理 `GET /api/workflows/{workflowId}/input-images/{imageId}`，浏览器仍只访问本 Java 服务；本次不增加网页图片上传入口。输入图片不展示为步骤产出。

启动前需要准备：

1. Java 17 和 Maven 3.9 或更高版本。
2. Python 工作流网关，默认地址为 `http://127.0.0.1:8080`。

监控中心可以在网关暂时离线时启动，但查询任务和连接状态会提示不可用。

## 开发启动

本项目 macOS 使用 `mvnd`，Windows 使用 `mvn`。下方 PowerShell 示例用于 Windows；macOS 启动、测试、打包和格式化均使用 `mvnd`，路径使用 `/`。

从仓库根目录执行：

```powershell
Set-Location .\services\workflow-console

$env:CODEX_GATEWAY_URL = "http://127.0.0.1:8080"
$env:SERVER_ADDRESS = "0.0.0.0"
$env:SERVER_PORT = "8090"

mvn spring-boot:run
```

查看指定任务：

```text
http://127.0.0.1:8090/?workflowId=<workflowId>
```

URL 没有 `workflowId` 时，页面只显示任务编号输入框，不展示全局任务列表。

## 打包运行

```powershell
mvn package

$env:CODEX_GATEWAY_URL = "http://127.0.0.1:8080"
java -jar .\target\workflow-console-0.1.0.jar
```

## 环境变量

| 变量 | 默认值 | 说明 |
| --- | --- | --- |
| `SERVER_PORT` | `8090` | 服务端口 |
| `SERVER_ADDRESS` | `0.0.0.0` | 监听地址 |
| `CODEX_GATEWAY_URL` | `http://127.0.0.1:8080` | Python 网关地址 |

## 增量刷新

监控页在活动状态下约每 2 秒检查一次，终态改为每 15 秒轻量检查；回到前台或重新聚焦时立即检查，能够发现其他入口发起的返工。查询携带状态和步骤结果版本，未变化时复用缓存，不重新加载长结果或重建未变化的步骤、消息元素。

消息首次只读最近 200 条可见事件，“加载更早消息”按游标回看；每轮增量刷新最多一页，底层审计事件仍保存在中央运行库。长期持续输出时限制自动保留的事件数量，较早消息可再次加载。

内网 HTTP 页面使用 `crypto.getRandomValues` 生成 UUID 的兼容方案；失败重试仍复用原消息编号。监控中心不增加直接运行控制接口，也不连接 MySQL。

## 业务接口

系统提供五个读取接口、一个聊天消息接口和两个半自动流转接口：

```text
GET /api/gateway/ready
GET /api/workflows/{workflowId}
GET /api/workflows/{workflowId}/events?after=0&limit=200
GET /api/workflows/{workflowId}/artifacts/{artifactId}
GET /api/workflows/{workflowId}/input-images/{imageId}
POST /api/workflows/{workflowId}/messages
POST /api/workflows/{workflowId}/advance/{gateId}/confirm
POST /api/workflows/{workflowId}/advance/{gateId}/hold
```

消息请求示例：

```json
{"messageId":"浏览器生成的 UUID","text":"第1步还没结束吗？"}
```

相同 `messageId` 的失败请求应复用原 UUID 重试。所有终态仍可继续咨询；也可在额度允许时请求从某一步重新执行到末尾。

其中事件接口代理 Python 网关的历史事件查询，并支持通过 `after` 增量读取。`limit` 的允许范围为 `1` 到 `1000`；可选 `view=monitor`、`tail=true` 或 `before`，响应包含下一页游标与是否存在更早消息。状态接口可选 `knownRevision`、`knownResults`，语义见 Python 模块 README。

文件接口只代理 Python 网关已经归属到该工作流的附件，不接受本机文件路径。图片使用 `inline` 预览；其他类型强制 `attachment` 下载，所有响应均带 `X-Content-Type-Options: nosniff`。浏览器不会直接访问 Python 网关或执行机文件系统。

## 状态显示

| 内部状态 | 页面显示 |
| --- | --- |
| `pending` | 尚未开始 |
| `queued` | 等待开始 |
| `running` | 正在进行 |
| `completed` | 已完成 |
| `skipped` | 已跳过 |
| `failed` | 未完成 |
| `cancelled` | 已停止 |
| `interrupted` | 已中断 |
| `cancelling` | 正在停止 |

## 测试

测试与格式化命令统一见[根 README](../../README.md#测试与格式化)，从仓库根目录执行并按 Windows/macOS 选择命令。

测试会启动 Spring 上下文，并验证 `/api` 下只有五个 GET、一个消息 POST 和暂停、继续两个半自动流转 POST 路由，不存在直接提交、取消、重试、跳过或编辑接口。

## 安全说明

系统没有登录功能，默认监听 `0.0.0.0` 以支持可信内网访问；应通过主机防火墙限制来源，不得将本系统或 Python 网关直接暴露到公网。如只需本机访问，可设置 `SERVER_ADDRESS=127.0.0.1`。

## 验收阶段展示

步骤可显示检查中、修复中、等待处理及验收通过，包含修复次数、原因与历次检查记录。验收等待不会倒计时放行；在任务助手中补充要求并独立确认追加一次修复和复检。监控中心仅代理查询及消息，不增加验收的直接跳过或重试接口。
