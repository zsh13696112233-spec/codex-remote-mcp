...
class RemoteEventBatcher:
    """保留现有异步事件接口，并通过中央 API 批量提交。"""
    ...
    async def _flush_batch(self) -> None:
        async with self._lock:
            ...
            try:
                await asyncio.to_thread(self.store.add_events, pending)
            except Exception:
    ...
    def _start_scheduled_flush(self) -> None:
        ...
        task = asyncio.create_task(
            self._run_scheduled_flush(), name="remote-workflow-event-flush"
        )
