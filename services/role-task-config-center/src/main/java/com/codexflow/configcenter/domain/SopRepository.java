...
interface SopRepository extends JpaRepository<SopEntity, String> {
  ...
  List<SopEntity> findByDeletedFalseAndGroupIdAndNameContainingIgnoreCaseOrderByCreatedAtDesc(
      String groupId, String query);

  List<SopEntity> findByDeletedFalseAndGroupIdIsNullAndNameContainingIgnoreCaseOrderByCreatedAtDesc(
      String query);
