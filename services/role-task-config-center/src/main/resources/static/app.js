...
function renderDingTalkTargets(){
  ...
  $("#content").innerHTML=`<section class="target-panel">
    <div class="settings-heading"><div><h2>钉钉通知对象</h2><p>一个群或人员可接收多个任务的通知；通知配置不限制机器人使用权限。</p></div>${configured?'<span class="connection-status online"><i></i>应用已配置</span>':'<span class="connection-status off"><i></i>请先配置机器人</span>'}</div>
...
function targetCard(x){
  ...
  const names=state.tasks.filter(t=>t.dingtalkTargetId===x.id).map(t=>t.name).join("、");
  const owner=names?{name:names}:null;
...
function blankSop(){return{groupId:concreteGroup(),id:"",name:"",description:"",supervisorAgentId:(suggestedAgents("supervisor")[0]?.agentId||""),supervisorTimeoutSec:7200,maxRetryCount:10,advanceMode:"automatic",handoffMode:"legacy_text",dingtalkShowExecutionDetails:false,defaultStepModel:"gpt-5.6-sol",enabled:true,steps:[]}}
