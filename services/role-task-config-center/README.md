...
钉钉启动的任务有会话回复地址时，等待通知沿用助手回答的文字回复链路，在群内 @ 发起人，由钉钉客户端展示原生回复交互；没有会话地址的主动通知仍发普通文本。等待通知的发送和回执失败均沿用 Outbox 重试，普通助手回复仍不补发。等待期间不展示主监督的执行文字、思考摘要和工具进度，用户提问的助手回答照常发送。消息消费和投递前均检查等待状态，并按步骤依赖及实际完成、启动时间过滤延迟到达的等待期间消息；步骤实际启动以 `node.started` 事件为准，不把主监督发出派发请求显示成已开始执行。

保持等待通知同样合并正文与 @ 提醒，只发送一条“已保持等待，不会自动进入下一步”的完整消息，不再追加“任务状态已更新，请查看上方消息”。问题的实际回答仍单独发送；保持等待也可能来自暂停操作，因此该通知不声称助手已经开始处理问题。

等待邀请正文增加“第 N 次等待确认”，按该工作流已持久化的等待通知编号，发送重试不改号。返工后即使步骤名称相同，引用正文仍能唯一匹配对应等待；旧引用不能确认新等待。历史已发送的相同正文无法补改，匹配有歧义时仍拒绝猜测，可用工作流编号加“确认继续”定位当前等待。

钉钉明确回复“确认继续”时只登记对应等待并确认放行，不触发“已保持等待”通知，也不先暂停主监督。提问、图片和暂停请求仍先保持等待。此调整需先更新 Python 网关，再更新配置中心。
