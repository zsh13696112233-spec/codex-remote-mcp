...
- 可选机器管理：在 `CODEX_AGENT_SOURCE=registry` 部署下，先建组，再按 IP、端口和能力登记机器，手动检测后用于运行；同一机器允许兼任主监督与执行机。支持分组重命名、空组删除、机器编辑启停和一次性旧配置导入，不提供机器物理删除或运行中配置锁定。
- 登记模式下 SOP 保存会经网关校验机器能力和同组关系，步骤建议列表按主监督所在组过滤；提交时再验证检测资格。下面关于任意 ID 和离线保存的说明仅适用于默认文件模式。配置和新增管理接口见[机器管理部署说明](../../docs/WEBUI_MACHINE_REGISTRATION.zh-CN.md)。
