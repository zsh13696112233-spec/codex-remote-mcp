...
使用最新配置创建的新运行会在提交 JSON 中固定写入 `handoffMode: "cumulative_files"`，步骤之间累计交接当前有效文件，不传递前序文字结果。按历史快照重试不会补写该字段：旧快照如果缺失字段，仍使用兼容的 `legacy_text` 行为。
...
如果此前已经导入过旧版示例模板，需要重新导入当前 JSON 并发布一个新版本，才能获得等待橙、完成绿、执行蓝、失败红的状态标题样式和新版预览；变量名未变化，因此模板 ID 不变时无需重新配置代码或机器人。
...
互动卡片会复用同一个卡片实例动态刷新，而不是每次进度变化都创建新卡。正文展示完成比例、当前步骤与角色、运行时长、重跑次数、各步骤状态、最近一个已完成步骤的产出摘要，以及最近一次任务助手回复的摘要；助手完整回复仍会单独发送，避免长内容挤满卡片。半自动倒计时或暂停期间使用“等待”视觉状态，完成、执行中和失败使用对应视觉状态。为避免网络恢复后旧更新覆盖新进度，过期的卡片 Outbox 更新会直接淘汰。
