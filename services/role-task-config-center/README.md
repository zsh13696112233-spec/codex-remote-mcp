...
- 机器管理：只支持网页登记。先建组，再按 IP、端口和能力登记机器，手动检测后用于同组任务；同一机器可兼任主监督和执行机。页面采用左侧分组、中间主监督机、右侧执行机的通栏紧凑列表布局，机器条目不显示角色文字头像；点击分组切换机器列表，双角色机器在两栏分别展示，新增机器默认使用当前分组和所在栏角色，窄屏依次堆叠。支持分组重命名、空组删除、机器编辑启停，不提供旧配置导入、物理删除或运行中配置锁定。统一部署参数见[机器管理部署说明](../../docs/WEBUI_MACHINE_REGISTRATION.zh-CN.md)。
