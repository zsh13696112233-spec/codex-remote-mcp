...
- 可选的飞书或钉钉长连接机器人：固定绑定一个任务定义。钉钉固定任务的 SOP 只能选择一个已启用的人员或群聊作为通知和控制对象；两个平台互斥启用。
...
3. 进入 8091 的“钉钉机器人”页面，填写 Client ID、Client Secret、固定任务和轮询间隔，先保存为停用状态。
4. 进入“钉钉通知对象”：人员由管理员点击“同步公司人员”手动拉取，首次同步默认停用；群聊在群内首次 `@机器人` 后自动登记，默认停用。管理员确认名称、测试发送并启用一个对象。
5. 编辑固定任务引用的 SOP，在“钉钉通知对象”中选择刚启用的一个人员或群聊，然后回到“钉钉机器人”页面启用机器人。未选择有效对象时系统会拒绝启用或启动。
...
- 一个机器人仍固定绑定一个任务定义，且全局同时最多运行一个机器人任务；通知对象由该任务引用的 SOP 决定，人员与群聊严格二选一且只能选一个。
- 只有从钉钉目标对象发起的运行会建立钉钉会话绑定和回推消息；从 8091 页面启动的同一任务不会回推钉钉。
...
飞书和钉钉配置接口只服务于受保护的 8091 内网页面。GET 和 PUT 的响应只返回 `secretConfigured`，不会返回 App Secret 或 Client Secret；测试接口也只返回成功状态和普通中文提示。人员同步由管理员手动触发，不设置定时任务。两个平台的启用状态互斥。
