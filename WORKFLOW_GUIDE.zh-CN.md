...
> 请先阅读 `C:\Users\13696\work\codex-remote-mcp\WORKFLOW_GUIDE.zh-CN.md`，然后继续处理工作流服务。
...
主监督会话所在的 Codex app-server 必须加载本项目 MCP：
...
$env:CODEX_AGENTS_FILE = "C:\services\codex-remote-mcp\agents.json"

python workflow_gateway.py `
  --host 0.0.0.0 `
  --port 8080 `
...
```text
codex_orchestrator_mcp.py   app-server 客户端和 MCP 节点工具
workflow_gateway.py         HTTP/SSE 网关和主监督会话
workflow_store.py           SQLite 工作流、节点和事件存储
agents.json                 当前执行机配置
agents.example.json         执行机配置示例
workflows.db                运行时数据库，已被 gitignore 忽略
README.md                   项目快速说明
WORKFLOW_GUIDE.zh-CN.md     本文档
tests/                      自动化测试
