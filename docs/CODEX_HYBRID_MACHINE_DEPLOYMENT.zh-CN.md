...
当前统一凭据部署让所有 `4500` app-server 使用同一个共享 app-server Token，保存在 Git 之外，并在中央和所有远程主监督上使用统一客户端引用路径。每台执行服务使用不同令牌需要另外设计凭据映射，不属于本文的统一部署流程。
...
配置文件角色表、一体机 JSON 示例、两个 `token_file` 的区别及远程缺少文件的补齐步骤，统一见[机器配置与两种凭据](WEBUI_MACHINE_REGISTRATION.zh-CN.md#一体机配置与两种凭据)。特别注意：远程 `sidecar.token_file` 不是派发步骤时使用的执行服务凭据引用；后者由中央同组清单提供。
