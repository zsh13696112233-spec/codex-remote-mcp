...
`8091` 定义任务并向 `8080` 提交，也可通过飞书或钉钉官方 SDK 长连接接收启动和控制消息；钉钉限定为 SOP 选择的唯一人员或群聊，两个机器人平台互斥启用。`8080` 调度主监督和各执行步骤，`8090` 从 `8080` 查询并展示指定工作流；MySQL 保存配置、运行快照和机器人可靠投递状态，SQLite 保存实际运行状态。
...
flowchart LR
    ...
    BOT[飞书群聊或钉钉指定人员/群聊] -->|官方 SDK 主动长连接<br/>HTTPS/WSS 443| C
...
- 飞书或钉钉机器人页面配置、钉钉人员/群通知对象、SOP 的唯一钉钉目标、平台互斥状态、全局执行槽、会话绑定、入站消息幂等映射、事件游标和发送 Outbox。钉钉会话绑定冻结目标类型和外部 ID，后续配置变更不改变已运行任务；只有钉钉发起的运行建立此绑定。
