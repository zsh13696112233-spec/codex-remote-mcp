...
[`sop等待选择卡片_1788953254627.json`](sop等待选择卡片_1788953254627.json) 用于步骤间等待通知与回答正文。保留卡片头部和 Markdown 正文，底部只保留“继续执行”；已删除自定义“回复”按钮及其专用变量。用户已确认前一版模板可显示并已发布，本次单按钮版本仍需重新导入、保存、发布和客户端验收。
...
配置中心已接入此单按钮模板；需重新构建并重启后生效。原生图文引用和真实钉钉投递仍需客户端验收。
